...
add_action( 'after_setup_theme', 'NirTheme_woocommerce_support' );
if ( ! function_exists( 'NirTheme_woocommerce_support' ) ) {
	/**
	 * Declares WooCommerce theme support.
	 */
	function NirTheme_woocommerce_support() {
		...
		add_filter( 'woocommerce_form_field_args', 'NirTheme_wc_form_field_args', 10, 3 );
		add_filter( 'woocommerce_form_field_radio', 'NirTheme_wc_form_field_radio', 10, 4 );
		add_filter( 'woocommerce_quantity_input_classes', 'NirTheme_quantity_input_classes' );
		add_filter( 'woocommerce_loop_add_to_cart_args', 'NirTheme_loop_add_to_cart_args' );

		// Wrap the add-to-cart link in `div.add-to-cart-container`.
		add_filter( 'woocommerce_loop_add_to_cart_link', 'NirTheme_loop_add_to_cart_link' );

		// Add Bootstrap classes to account navigation.
		add_filter( 'woocommerce_account_menu_item_classes', 'NirTheme_account_menu_item_classes' );
	}
}
...
add_action( 'woocommerce_before_main_content', 'NirTheme_woocommerce_wrapper_start', 10 );
add_action( 'woocommerce_after_main_content', 'NirTheme_woocommerce_wrapper_end', 10 );

if ( ! function_exists( 'NirTheme_woocommerce_wrapper_start' ) ) {
	/**
	 * Display the theme specific start of the page wrapper.
	 */
	function NirTheme_woocommerce_wrapper_start() {
...
if ( ! function_exists( 'NirTheme_woocommerce_wrapper_end' ) ) {
	/**
	 * Display the theme specific end of the page wrapper.
	 */
	function NirTheme_woocommerce_wrapper_end() {
		echo '</main>';
...
if ( ! function_exists( 'NirTheme_wc_form_field_args' ) ) {
	/**
	 ...
	 */
	function NirTheme_wc_form_field_args( $args, $key, $value ) {
		$bootstrap4 = 'bootstrap4' === get_theme_mod( 'NirTheme_bootstrap_version', 'bootstrap4' );
...
if ( ! function_exists( 'NirTheme_wc_form_field_radio' ) ) {
	/**
	 ...
	 * `NirTheme_wc_form_field_args()` that is meant for input labels only.
	 *
	 ...
	 * @return string
	 */
	function NirTheme_wc_form_field_radio( $field, $key, $args, $value ) {
...
if ( ! function_exists( 'NirTheme_quantity_input_classes' ) ) {
	/**
	 ...
	 * @return array
	 */
	function NirTheme_quantity_input_classes( $classes ) {
		$classes[] = 'form-control';
		return $classes;
	}
}

if ( ! function_exists( 'NirTheme_loop_add_to_cart_link' ) ) {
	/**
	 ...
	 */
	function NirTheme_loop_add_to_cart_link( $html ) {
		return '<div class="add-to-cart-container">' . $html . '</div>';
	}
}

if ( ! function_exists( 'NirTheme_loop_add_to_cart_args' ) ) {
	/**
	 ...
	 */
	function NirTheme_loop_add_to_cart_args( $args ) {
...
if ( ! function_exists( 'NirTheme_account_menu_item_classes' ) ) {
	/**
	 ...
	 */
	function NirTheme_account_menu_item_classes( $classes ) {
